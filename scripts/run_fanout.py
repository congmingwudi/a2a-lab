...
async def main() -> int:
    ...
    # Leg selection is done by temporarily narrowing the registry rather than
    # by filtering inside dispatch(): the production path always runs every
    # leg, and a --legs flag that changed dispatch's behaviour would mean the
    # thing under test is not the thing that ships.
    import orchestration.legs as legs_mod

    original = legs_mod.LEGS
    if wanted:
        legs_mod.LEGS = tuple(leg for leg in original if leg.role in wanted)
    try:
