...
Pick the least complex Claude surface that meets the runtime requirement.
Hosted sessions, a self-hosted agent harness, and a direct Messages API loop
trade off control, security boundary, and operational ownership differently —
and the lab needed all three answers.
...
- **Repository-backed:** the managed backend, the six-tool Lab Guide loop, and
  the analyst entry point are all present in the referenced files; D35 records
  the guide design, D23 the hosted analyst.
- **Vendor-documented:** Managed Agents provides persistent agent/environment
  resources and host-executed custom tools. It is a beta API — re-check
  versioned details before reusing this as a sample.
- "Curated tools beat RAG" is **conditional**, and the slide should say so: it
  holds here because the corpus is small, structured, and authoritative. It is
  not a general argument against retrieval.
...
|---|---|---|
| Hosted sessions and sandboxing | Managed Agents | Less runtime to own; host-side credential boundary |
| Container and runtime control | Agent SDK | Portable agent interior; the infrastructure stays yours |
| A small, transparent tool loop | Messages API | Minimal abstraction, visible control flow |

**Visual:** the table, or three runtime boxes side by side. Lead with the
decision criteria, not a feature inventory — the memorable line is that the
same lab needed all three, and the adapter seam is why swapping them is
invisible.
