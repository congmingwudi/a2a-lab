...
**What the lab showed:** The lab's Path A budget: Agentforce action ~85-90s (MEASURED 2026-07-25 — the ~60s the lab engineered against for six weeks was a reported figure nobody had tested; an action returning at 84.7s was used, one at 89.7s was abandoned) → Apex callout 110s → bridge 45s → remote agent 100s (`CLAUDE_ANSWER_TIMEOUT_S`; raised from 40s, which proved too tight the moment a delegated turn contained a platform round trip — 40 survives only as an in-code fallback the shipped .env overrides). Worth naming that the correction went the generous direction and still cost something: the budget was tuned to buy research depth against a ceiling 25s lower than the real one. The delegated agent's thinking depth is governed by the smallest budget upstream, and one platform's retry can blow another's ceiling. When the ceiling IS hit, nothing on the wire says so: the Agent API returns 200, the Agentforce agent writes its "External market research (from the Claude research agent):" heading exactly as on a good turn, and fills it with "External research is temporarily unavailable" — while the delegated agent's real answer arrives seconds later and is discarded. Every abandoned turn still burned the full ~100s. The rule recurses: hosting the lab's A2A shim behind API Gateway added a hard 29s ceiling that an Agentforce account turn (~20-27s tail, D32) straddles — every intermediary you add contributes its own timeout to the stack. And third-party callers hit it harder than your own code (2026-07-22): the lab's client retries once onto a warmed session, but Microsoft Foundry's A2A tool does not retry — a cold turn behind the gateway 500s straight into the calling agent. You control your retries; you don't control your callers'.
...
**What the lab showed:** Every A2A exchange in the lab returns a Task object with an id, state machine, and artifacts (the lab's protocol mapping records the completed-Task/one-artifact shape and TASK_STATE_FAILED on errors). For months the lab drove that lifecycle synchronously and assumed the server was the reason. MEASURED 2026-07-27, it was not: the a2a-sdk already honours `configuration.return_immediately`, responding once the Task exists while a background task drains the queue, and the lab had simply never set the field. No server change was required. With it set, `submit()` returns in 13–29ms against adapter work of 2s, 10s and 45s — flat while the work grows 22x — and the hosted Agentforce shim completed 31.1s of work behind an API Gateway whose integration timeout is 29s, longest single request 1.18s. Streaming remains the genuinely unexercised capability: the servers advertise `AgentCapabilities(streaming=True)`, the client hard-codes `ClientConfig(streaming=False)`, and D11 scoped an SSE demo that never ran. Vendor support for the async half is uneven and only measurement reveals it — Azure Foundry implements it; Vertex AI Agent Engine returns a task id in 826ms that no request shape reads back.
...
**What the lab showed:** MEASURED 2026-07-27. A2A specifies that SendMessage MUST return immediately and processing MAY continue afterwards, and the lab had implemented that lifecycle — InMemoryTaskStore, the full SUBMITTED → WORKING → COMPLETED state machine — then driven it synchronously for months. The workstream that set out to fix this named the server as the blocker (an executor awaiting the adapter inline) and was WRONG: the a2a-sdk already reads `configuration.return_immediately` and, when set, responds after the first Task event while a background task drains the queue. No server change was made. Setting one field on the client took the hosted Agentforce shim from work bounded by API Gateway's 29s integration timeout to 31.1 SECONDS of work completed behind it, with a longest single request of 1.18s — dissolving the ceiling that D41 had costed out an apigatewayv2→v1 migration and a quota request to raise. Against a deterministic slow adapter, submit latency is flat at 13–29ms while the work grows from 2s to 45s. Support is uneven and only measurement reveals it: Azure Foundry implements the async half properly (ratio 0.25–0.33); Vertex AI Agent Engine is submit-only, returning a task id in 826ms that no tried request shape reads back (GET /tasks/{id} is rejected with "A2A version '0.3' is not supported by this handler"), so an async submit there silently loses the answer.
...
**What the lab showed:** MEASURED 2026-07-27 on the lab's hosted Agentforce A2A shim (Lambda behind API Gateway). Polled steadily, a 31.1s job completed in 21 polls with no request longer than 1.18s. Submitted and then left QUIET for 45 seconds — comfortably longer than the ~30s the underlying work takes — the task was still WORKING on the first poll, and completed only after 12 further polls at t+67.7s. Lambda freezes the execution environment when the response is sent, so the background consumer gets CPU only while an invocation is in flight: the polls are not observing the work, they are paying for it. Total wall-clock got WORSE with less polling, which inverts ordinary polling advice. The same pattern against always-on hosts (Azure Foundry, the lab's own Fargate bridge) progresses whether or not anyone is looking. A second limit rides along — the task lives in an in-memory store in one warm container, and nothing guarantees a later poll reaches that instance.
