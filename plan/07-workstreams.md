...
*Status 2026-09-06:* built test-first on `ws25-b1-security-seams`; Codex's cross-review (REQUEST CHANGES: three A1 leak shapes, the obs bundle cold start, ARN-only guards, three A2 decisions) answered in round 2 — every finding fixed or adopted, see `build-notes/claude/reviews/ws25-b1.md`. Awaiting Codex's round-2 verdict. Deploys run from the operator's work machine — this machine cannot reach the lab's AWS account — per the runbook in `build-notes/claude/reviews/ws25-b1.md`. Items flip to ✅ after the cross-review verdict AND the deploy.
...
2. ⏳ A2 — viewer role enforced on the real invoke surface (`/invoke`, `/invocations`, MCP `tools/call`, A2A v1 `SendMessage` and the 0.3 spelling) by one policy function in `src/interop/authz.py`, called from the REST handler and from the WireTap's single body buffer for MCP/A2A; discovery and task reads stay open; `machine` allowed at the faces (the gateway fronts every face), denied at the console; refusals recorded as error hops; `DELETE /api/traces` added to the console operator gate; role copy updated (F01, D36).
...
*Status 2026-09-09:* settled as handoff batch `ws25-b3` (D81); built test-first on `ws25-b3-crm-writes-sessions`. A4 replaces the watcher's id-only "serviced" set with a per-session outcome ledger (`status`/`attempts`/`calls`, keyed by tool_use_id) persisted to the hosted state store, gates the host-side `save_brief` on that ledger, and adds an operator-only `/api/briefs/outcomes` console reader; delivery is made idempotent by upserting `A2ALab_Account_Brief__c` + `Task` on a composite external-id `A2ALab_Delivery_Key__c` = `{research_session_id}#{account_id}` (Metadata API deploy of two fields + the permission-set grant). A5 makes account resolution deterministic — reject empty, exact `WHERE Name` then a single escaped `LIKE`, ambiguous listed, with an optional preauthorized `A2ALAB_BRIEF_ACCOUNT_MAP` id (binding-checked). A11 threads the JWT subject the auth middleware verified through a contextvar into the executor, keys the reused shim session by `(platform, verified subject)` — never a caller-asserted body field — and adds a per-key lock in `AgentforceClient.ensure_session`. Awaiting Codex's cross-review verdict; deploys (Faces, Shim, Console, hosted watcher/runner, Salesforce Metadata API after a composite-key duplicate scan) run from the operator's work machine per the runbook in `build-notes/claude/reviews/ws25-b3.md`. Items flip to ✅ after the cross-review verdict AND the deploy.
