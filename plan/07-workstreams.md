...
*Status 2026-09-06:* built test-first on `ws25-b1-security-seams` (A1, A3, A10, CI, and the console half of A2); the auth-middleware half of A2 awaits Codex's design critique (`build-notes/claude/ws25-a2-auth-design.md`). Deploys run from the operator's work machine — this machine cannot reach the lab's AWS account — per the runbook in `build-notes/claude/reviews/ws25-b1.md`. Items flip to ✅ after the cross-review verdict AND the deploy.
